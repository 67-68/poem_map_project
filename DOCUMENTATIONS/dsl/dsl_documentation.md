...
>
> 💡 **`scan_and_push` 扫描逻辑**: 遍历全部 `Database.random_events` 桶，对每个事件的 `target_tags` 与输入 tags 做前缀匹配（`TagManager.prefix_match`），符合条件的生成 `EventTicket` → 通过 `RequirementFilter` / `ActionTagFilter` 过滤 → 按权重滚动选中 → 推入栈顶。权重基础值（1.0）乘以 `weight_mult` 系数。未匹配到任何事件时触发 `fallback` 兜底事件。
>
