...
| `val=50` | int | 纯数字 |
| `val=3.14` | float | 带小数点 |
| `val=m_money_gain` | int (Named Amount) | **命名数值符号**，从 `tools/data/named_amounts.json` 查表解析 |
...
>
> 💡 **Named Amount（命名数值）**：`val` 参数支持使用语义别名替代魔法数字。例如 `val=m_money_gain` 会被查表解析为 `30`。符号表定义在 `tools/data/named_amounts.json`，Python 管道和 Godot 运行时双端生效。命名规范：`{magnitude}_{domain}`（如 `m_money_gain`, `m_health_cost`）。
...
| `clear_scheduled_events` | 无参数 | 清空事件栈和队列中所有待处理事件，强制中断当前事件链 | `clear_scheduled_events()` |
...
> ⚠️ ` and ` 前后必须带空格，避免误匹配变量名中可能含有的 `and` 子串（如 `expand`、`command`）。
