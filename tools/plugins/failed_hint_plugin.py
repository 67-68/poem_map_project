...
3. 必须出现一个未遂的动作——但不限于肢体动作。方向池（每次随机选择，不得连续重复同一方向）：
   - 肢体型：手/脚/肩/背/下颌/后颈 的发起与撤销（禁止 opt_kuangke 只用「手」、禁止 opt_fengying 只用「腰/膝盖」）
   - 信息型：话到嘴边但内容被置换、信已写好但署名被涂改、已递出的名帖中途改为袖回、已推过桌面的物件在最后一寸停住
   - 空间型：已走到门口但绕了回来、已推开半扇门但只是去取一件东西、已在楼梯上转身但只是换了一级台阶
   - 沉默型：沉默本身即是动作的撤回——准备开口但最终只用眼神回应、已吸入一口气但呼出时变成了别的话
   - 物品型：银两已推过桌心但用茶碗压住、笔已蘸墨但只在纸上点了一个点、布袋已掂在手里但放回原处
4. 禁止纯环境描写、禁止纯生理反应（咳血、发抖、腿软）、禁止没有性格判断的犹豫

【Good Case — 结构骨架（禁止照抄具体动作，只参照三步结构）】
结构：触发物 → 本能反应（不限于手/腰/膝盖——可以是目光、脚步、呼吸节奏、沉默、指尖在桌面上的停顿）→ 转折词 → 性格判断 → 该反应被撤回或变形 → 外部世界的后果照常发生。

三步都在说「这是你选的性格，这是你选的后果。」具体执行时，本能反应和撤回方式必须与场景中的物理物品/空间关系绑定，不得复用「手伸出/收回」「腰弯/直起」。

【Good Case 示例 — 信息型未遂】
「话已经在嗓子眼——那句'你找错人了'几乎脱口而出。但他不是无牵无挂的游侠。这句说出去，三天前他在赵主事门前递的那封信就成了废纸。最终嘴里吐出来的是另一个价码。」
→ 好在哪里：本能反应是语言（非肢体），撤回方式是内容置换，后果锚定在具体钻营史上。

【Good Case 示例 — 空间型未遂】
「鞋底已经转了半圈——门槛外的日光晃了一下。但他不是那种可以一走了之的人。这张雅集的门帖是他用三封密信换的，转身就是撕毁自己的账本。转回去的那半步把他的影子重新钉在门槛内。」
→ 好在哪里：本能反应是转身（非手/腰），撤回方式是半步偏移，后果锚定在入场券的交易史上。

【Bad Case — 因果链断裂】
「他站在门槛内，想迈出那一步，却想起昨夜咳出的血丝，于是退回屋里，轻轻掩上了门。」
→ 坏在哪里：只描述了犹豫。咳出的血丝是生理细节，不是性格因果。玩家不知道这是自己选的。

【Bad Case — 动作坍缩模式（严格禁止）】
「他的手已经抬到桌沿——但他不是一个不计后果的人。上一次在韦府甩脸子，他在长安等了三年……手指一根一根收了回来。」
→ 坏在哪里：又是「手伸出 + 收回」模板。这是第 N 次用同一个动作原型。禁止所有 failed_hint_opt_kuangke 以「手」的伸出/收回为动作主体，禁止所有 failed_hint_opt_fengying 以「腰/膝盖」的弯坠/直起为动作主体。方向池见铁律第 3 条——每生成一条新的 failed_hint 时必须切换到新的方向。
"""
