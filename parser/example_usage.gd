...
func test_batch_parsing():
    ...
    for event in events:
        print("- 事件: %s, 选项数: %d" % [event.id, event.options.size()])
...
func test_dsl_formats():
    ...
    # 测试属性需求解析
    var prop_req = MicroDSLParser.parse_property_requirement("prop:money:>50")
    if prop_req:
        print("属性需求: %s %s %d" % [prop_req.property, ">", prop_req.value])
    
    # 测试特性需求解析
    var trait_req = MicroDSLParser.parse_trait_requirement("trait:has:official")
    print("特性需求解析: ", trait_req != null)
    
    # 测试结果操作符解析
    var operators = MicroDSLParser.parse_consequence_operators("prop:money:-100,trait:add:corrupt")
    print("结果操作符数量: ", operators.size())
    
    # 测试复合需求解析
    var complex_req_str = "prop:money:>50,prop:literary_fame:>30"
