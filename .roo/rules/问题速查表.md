...
2. 意象分类。存在Detailed Imaginary/四段tag，AbstractConcept/Category/两段tag 和意象本身三种类别：形似TARGET:ACTOR:DUFU:STH, 两段意象形似ACTOR:DUFU, tag的中间两段就是意象/抽象意象。四段的tag也可以被叫做具体意象。意象本身存储一个字段，包含一堆的detailed imaginary, 模仿真实意象多态的特点。意象本身下属的detailed imaginary 的 第四段就是他自己的uuid
