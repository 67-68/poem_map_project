...
static func prob_line(prob: int) -> String:
	return color_size(tr("CODE_BBCODE_E18ED459F1") % prob, COLOR_MUTED, 13)


## 时间消耗行 — 灰色 13px
static func time_cost_line(cost_detail: String) -> String:
	return color_size(tr("CODE_BBCODE_2FB48A6C2F") % cost_detail, COLOR_MUTED, 13)


## 时间范围行 — 灰色 13px，如「⏱ 耗时 5天 ～ 8天」
static func time_span_line(min_detail: String, max_detail: String) -> String:
	return color_size(tr("CODE_BBCODE_EAE89B1E8B") % [min_detail, max_detail], COLOR_MUTED, 13)


## 前提 section 标题 — 灰色 13px 的「━━━ 前提 ━━━」
static func req_section() -> String:
	return hint_section(tr("CODE_BBCODE_B9C206F855"))


## 结果 section 标题 — 灰色 13px 的「━━━ 结果 ━━━」
static func result_section() -> String:
	return hint_section(tr("CODE_BBCODE_A95858CB62"))


## 失败 section 标题 — 灰色 13px 的「━━━ 失败 ━━━」
static func fail_section() -> String:
	return hint_section(tr("CODE_BBCODE_ECCD17434E"))


## 持续 section 标题 — 灰色 13px 的「━━━ 持续 ━━━」
static func duration_section() -> String:
	return hint_section(tr("CODE_BBCODE_9B4C29C5F6"))


## 效果 section 标题 — 灰色 13px 的「━━━ 效果 ━━━」
static func effect_section() -> String:
	return hint_section(tr("CODE_BBCODE_F17C55154D"))
...
static func repeated_warning() -> String:
	return color(tr("CODE_BBCODE_4F9DFB5567"), COLOR_WARNING)
...
static func defer_waiting(remaining: int, total: int, is_failing: bool) -> String:
	...
	return color_size(tr("CODE_BBCODE_01EEBE490C") % [remaining, total], c, 13)
...
static func defer_pending(xun_val: int) -> String:
	return color_size(tr("CODE_BBCODE_C1E9B923E7") % xun_val, COLOR_MUTED, 13)


## Defer 失败后果行
static func defer_failing(fb_msg: String) -> String:
	return color(tr("CODE_BBCODE_85C48E965A") % fb_msg, COLOR_DANGER)
...
static func place_hint(place_name: String) -> String:
	return color_size(tr("CODE_BBCODE_7117939F67") % place_name, COLOR_PLACE, 13)


## SIMPLE profile 异地行动提示 — place 色，简版「赴%s」
static func simple_place_hint(place_name: String) -> String:
	return color_size(tr("CODE_BBCODE_7F0CC21586") % place_name, COLOR_PLACE, 13)


## 时间不足行
static func time_insufficient(cost_detail: String, remaining_days: int) -> String:
	return color(tr("CODE_BBCODE_FA6F390B49") % [cost_detail, remaining_days], COLOR_DANGER)


## 子行动预览头部
static func preview_header() -> String:
	return tr("CODE_BBCODE_A8DE0CCC7F")


## 子行动概率行（非 BBCode 纯文本）
static func sub_prob_line(prob: int) -> String:
	return tr("CODE_BBCODE_BB986C4BCD") % prob


## 子行动成功效果标题
static func success_header() -> String:
	return tr("CODE_BBCODE_0C8ABDDBB1")


## 子行动失败效果标题
static func fail_header() -> String:
	return tr("CODE_BBCODE_8B1564F7D5")


## Imaginary 等级标签
static func imaginary_level(level: int, description: String) -> String:
	var base := tr("CODE_TRAIT_HINT_FORMATTER_D8663A39B4") % level
...
static func simple_feasibility_label(text: String) -> String:
	return tr("CODE_BBCODE_1726614F08") + text if not text.is_empty() else tr("CODE_BBCODE_2C5F8B18A4")


## SIMPLE profile: 消耗标签 — "耗：⏱3天 赴洛阳"
static func simple_cost_label(lines: Array[String]) -> String:
	var joined = " ".join(lines)
	return tr("CODE_BBCODE_703E75446D") + joined if not joined.is_empty() else tr("CODE_NPC_ACTION_BUTTON_7CA5205A5B")


## SIMPLE profile: 产出标签 — "产：健康↑ 金钱↑↑"
static func simple_output_label(lines: Array[String]) -> String:
	var joined = " ".join(lines)
	return tr("CODE_BBCODE_B082955862") + joined if not joined.is_empty() else tr("CODE_NPC_ACTION_BUTTON_0F9928F317")


## SIMPLE profile: 风险标签 — "险：后果难料…"
static func simple_risk_label(lines: Array[String]) -> String:
	var joined = " ".join(lines)
	return tr("CODE_NPC_ACTION_BUTTON_4D7C36AA23") + joined if not joined.is_empty() else tr("CODE_NPC_ACTION_BUTTON_4D7C36AA23")


## SIMPLE profile: 锁定标签 — 红色 "锁定：条件不足"
static func simple_lock_label(reason: String) -> String:
	var text = tr("CODE_BBCODE_0D46022B0E") + reason if not reason.is_empty() else tr("CODE_BBCODE_E9BCBC440D")
