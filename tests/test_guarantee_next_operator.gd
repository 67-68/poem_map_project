...
#   - EventManager._on_guarantee_next 存储 event_key + main_tag（FIFO队列）
#   - roll_events 三路分支：
...
func before_each():
	# 重置 EventManager 的 guarantee 状态（FIFO 队列）
	EventManager._guaranteed_events.clear()
...
# ════════════════════════════════════════════════════════════
# 测试: EventManager 存储 guarantee（FIFO队列）
# ════════════════════════════════════════════════════════════

func test_event_manager_stores_both_values():
	"""EventManager._on_guarantee_next 应 push 到 FIFO 队列，同时存储 event_key 和 main_tag"""
	EventManager._on_guarantee_next("evt_stored", "STORED_TAG")
	
	assert_eq(EventManager._guaranteed_events.size(), 1, "FIFO 队列应有 1 个条目")
	assert_eq(EventManager._guaranteed_events[0].event_key, "evt_stored", "event_key 应被存储")
	assert_eq(EventManager._guaranteed_events[0].main_tag, "STORED_TAG", "main_tag 应被存储")


func test_event_manager_stores_empty_tag():
	"""main_tag 为空时应正确存储空字符串到 FIFO 队列"""
	EventManager._on_guarantee_next("evt_empty_tag", "")
	
	assert_eq(EventManager._guaranteed_events.size(), 1, "FIFO 队列应有 1 个条目")
	assert_eq(EventManager._guaranteed_events[0].event_key, "evt_empty_tag", "event_key 应被存储")
	assert_eq(EventManager._guaranteed_events[0].main_tag, "", "main_tag 应为空字符串")
...
func test_branch_a_no_tag_finds_item():
	...
	EventManager._guaranteed_events.append({"event_key": "evt_guaranteed", "main_tag": ""})
	...
	assert_eq(EventManager._guaranteed_events.size(), 0, "guarantee 应被消费")


func test_branch_a_no_tag_missing_item():
	"""分支A: 无 main_tag → find_triggerable_item 找不到 → 回退正常抽取"""
	EventManager._guaranteed_events.append({"event_key": "evt_nonexistent", "main_tag": ""})
...
func test_branch_b_tag_matches_finds_in_pool():
	...
	EventManager._guaranteed_events.append({"event_key": "evt_guaranteed", "main_tag": "TEST_BUCKET"})
...
func test_branch_c_tag_mismatch_preserves_guarantee():
	...
	assert_eq(EventManager._guaranteed_events.size(), 1, "guarantee 应被保留")
	assert_eq(EventManager._guaranteed_events[0].event_key, "evt_guaranteed", "guarantee event_key 应被保留")
	assert_eq(EventManager._guaranteed_events[0].main_tag, "TEST_BUCKET", "guarantee main_tag 应被保留")
...
func test_branch_c_tag_mismatch_allows_subsequent_match():
	...
	assert_eq(EventManager._guaranteed_events.size(), 1, "guarantee 应仍保留")
	...
	assert_eq(EventManager._guaranteed_events.size(), 0, "guarantee 应已被消费")
