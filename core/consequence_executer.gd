...
func execute_result(result: ChoiceResult):
    ...
    match result.action_type:
        ...
        "trait", "property", "multi_property", "time_change":
            result.operate()
        _:
            Logging.err('Unknown action type: %s' % result.action_type)
        
        
func _add_event(result: ChoiceResult):
    var next_item = Global.find_triggerable_item(result.target_event_uuid)
    if next_item:
        if next_item is FocusedChat or ChatBubble:
            ...
            Logging.warn('target: %s next: %s' % [result.target_event_uuid,next_item.uuid])
    else:
        Logging.warn('can not find a valid uuid for a triggerable item in data: %s' % result.target_event_uuid)
