...
static func build_operator_preview(operators: Array, profile := _HintProfile.Profile.DEFAULT) -> Array[String]:
	return _OperatorPreviewFormatter.build_preview(operators, profile)


## 🤓☝️ 绞杀者转发：业务已正式迁移至 OperatorPreviewFormatter
## @param profile: 提示模式 — HintProfile.Profile.DEFAULT（默认）或 SIMPLE
static func build_choice_result_preview(result: ChoiceResult, profile := _HintProfile.Profile.DEFAULT) -> Array[String]:
	return _OperatorPreviewFormatter.build_choice_result_preview(result, profile)
...
## 保持对 consumers 的向后兼容。
## @param profile: 提示模式 — HintProfile.Profile.DEFAULT（默认）或 SIMPLE
static func build_action_hint(action: Action, is_locked: bool, profile := _HintProfile.Profile.DEFAULT):
	if not action:
	...
	var result = _ActionHintFormatter.build_action_hint(action, is_locked, ctx, profile)
...
static func build_trait_hint(trait_data: Trait) -> String:
	# 🤓☝️ 绞杀者转发：业务已正式迁移至 TraitHintFormatter
	return _TraitHintFormatter.build_hint(trait_data)
...
static func build_modifier_effects_hint() -> String:
	return _ModifierHintFormatter.build_effects_hint()
