...
func _init() -> void:
	...
	if not should_sync:
		...
		print("WARNING: csv_cloud_sync_cli.gd 被调用但没有 --sync 参数，直接退出")
		print("用法: godot --headless -s core/csv_cloud_sync_cli.gd -- --sync [--prefer-local]")
		quit(0)
		return
	
	var sep = _get_sep("=", 60)
	print("\n" + sep)
	print("  CSV 云同步 CLI 入口")
	print("  prefer_local: %s" % prefer_local)
	print(sep + "\n")
	...
	if loader_script == null:
		push_error("无法加载 csv_cloud_loader.gd")
		quit(1)
		return
