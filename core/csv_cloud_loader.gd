...
func _ready():
	...
	if should_sync:
		Logging.info("===== CLI 模式启动 csv_cloud_loader =====")
		Logging.info("prefer_local_files: %s" % prefer_local)
...
func process_next_job() -> void:
	...
	if job_is_generated:
		...
		if local_content == "":
			Logging.err("生成事件 CSV 文件不存在: %s 💀" % job_save_path)
			Logging.info("请先运行 Python 生成脚本: poetry run python tools/generate_orthogonal_events.py")
		else:
	...
	if job_url.is_empty():
		Logging.err("任务 %s 的 URL 为空，跳过 💀" % job_name)
		_current_job_index += 1
		process_next_job()
		return
	...
	if prefer_local_files:
		var local_content = _read_local_csv(job_save_path)
		if local_content != "":
			Logging.info("✅ 本地CSV文件存在，跳过云端下载: %s" % job_save_path)
			_process_csv_data(local_content, job_save_path, job_data_type)
			_current_job_index += 1
			process_next_job()
			return
		else:
			Logging.info("⚠️ 本地CSV文件不存在（路径: %s），降级到云端拉取..." % job_save_path)
...
func save_raw_csv(csv_content: String, save_path: String) -> void:
	...
	if file:
		...
		file.close()
		Logging.info("原始CSV文件已保存到: %s" % save_path)
	else:
		Logging.err("无法保存CSV文件到: %s 💀" % save_path)
		Logging.err('是不是文件路径写错了?')
...
func _read_local_csv(save_path: String) -> String:
	if not FileAccess.file_exists(save_path):
		Logging.info("本地CSV文件不存在: %s" % save_path)
		return ""
	
	var file = FileAccess.open(save_path, FileAccess.READ)
	if file == null:
		Logging.err("无法打开本地CSV文件: %s 💀" % save_path)
		return ""
	
	var content = file.get_as_text()
	file.close()
	Logging.info("成功读取本地CSV文件: %s (%d 字节)" % [save_path, content.length()])
	return content
...
func _process_csv_data(raw_csv_string: String, save_path: String, data_type: String, tres_output_dir: String = "") -> void:
	...
	for resource in resources:
		if resource is RandomEvent:
			Logging.info("事件注入成功: %s" % resource.uuid)
		elif resource is Flag:
			Logging.info("标志位注入成功: %s" % resource.uuid)
		elif resource is Trait:
			Logging.info("特性注入成功: %s" % resource.uuid)
		else:
			Logging.info("资源注入成功: %s" % resource.resource_path)

	Logging.info("数据注入完成！共注入 %d 个资源 🤓☝️" % resources.size())

	# 调试：检查资源有效性
	if resources.is_empty():
		Logging.info("Warning: DSLParser 返回空资源数组，跳过保存")
		return

	# 调试：检查转换后的资源数组
	Logging.info("资源数组转换完成，大小: %d" % resources.size())
	for i in range(min(resources.size(), 3)):  # 只打印前3个
		var res = resources[i]
		Logging.info("资源[%d]: 类名=%s, resource_path=%s" % [i, res.get_class(), res.resource_path])
...
func fetch_events_from_cloud(url: String, save_path: String = "res://tests/", data_type: String = "random_event") -> void:
	...
	if exit_code != 0:
		Logging.err("网络请求发起失败！错误代码: %d, 大唐的信使死在路上了 💀" % exit_code)
		if retry_count < MAX_RETRIES:
			retry_count += 1
			Logging.info("Warning: 第 %d 次重试原始URL..." % retry_count)
			fetch_events_from_cloud(url, save_path, data_type)
		else:
			# 重试次数耗尽，跳过当前任务继续下一个
			Logging.info("Error: 重试次数耗尽，跳过当前任务 😭")
			_current_job_index += 1
			process_next_job()
		return
	...
	Logging.info("成功获取数据，大小: %d 字节" % raw_csv_string.length())
	...
	Logging.info("云端数据注入成功！系统活过来了 🤓☝️")
...
func save_resources_to_tres(resources: Array[Resource], folder_path: String) -> void:
	...
	if not DirAccess.dir_exists_absolute(folder_path):
		...
		Logging.info("创建文件夹: %s" % folder_path)
	...
	for resource in resources:
		...
		if resource == null:
			Logging.info("Warning: 跳过 null 资源")
			skipped_count += 1
			continue
		
		# 验证资源是否可以保存
		if not is_resource_savable(resource):
			Logging.info("Warning: 跳过无效资源 (类名: %s)" % resource.get_class())
			skipped_count += 1
			continue
		...
		var action_res_path = resource.resource_path
		if not action_res_path.is_empty() and action_res_path != "":
			var res_dir = action_res_path.get_base_dir()
			if res_dir != "." and not res_dir.is_empty():
				actual_folder_path = res_dir + "/"
				Logging.info("🆕 Action resource_path 路由: %s → %s" % [action_res_path, actual_folder_path])
		...
		if resource is RandomEvent:
			...
			if custom_params.has("store_to"):
				...
				if STORE_TO_PATH_MAP.has(store_key):
					...
					Logging.info("🆕 store_to 命中映射表: key=%s → path=%s" % [store_key, actual_folder_path])
				else:
					# 未命中映射表，直接使用 key 作为 res:// 相对路径
					actual_folder_path = "res://" + store_key.trim_prefix("/")
					Logging.info("🆕 store_to 未命中映射表，直接使用 raw key: key=%s → path=%s" % [store_key, actual_folder_path])
				if not actual_folder_path.ends_with("/"):
					actual_folder_path += "/"
				# 确保目标文件夹存在
				if not DirAccess.dir_exists_absolute(actual_folder_path):
					DirAccess.make_dir_absolute(actual_folder_path)
					Logging.info("创建 store_to 文件夹: %s" % actual_folder_path)
		...
		if resource.has_method("get") and resource.get("uuid") != null:
			...
			if uuid is String and not uuid.is_empty():
				base_filename = uuid
				Logging.info("使用 uuid 作为文件名: %s" % base_filename)
		...
		if base_filename.is_empty():
			...
			if base_filename == "Resource":
				Logging.info("Warning: 资源没有有效名称，跳过保存 (类名: %s)" % base_filename)
				skipped_count += 1
				continue
		...
		if not invalid_chars.is_empty():
			...
			Logging.err("资源名称包含非法字符: %s, 非法字符: %s, 拒绝保存" % [base_filename, invalid_chars_str])
			skipped_count += 1
			continue
		...
		if save_result == OK:
			Logging.info("保存资源到文件: %s" % file_path)
			saved_count += 1
		else:
			Logging.err("保存资源失败: %s, 错误代码: %d" % [file_path, save_result])
			skipped_count += 1

	Logging.info("成功保存 %d/%d 个资源  (跳过 %d 个)" % [saved_count, resources.size(), skipped_count])
...
func _import_generated_events_from_csv() -> void:
	...
	if generated_entries.is_empty():
		Logging.info("⚠️  DATA_MANIFEST 中没有 is_generated 条目")
		Logging.info("   请先在 DATA_MANIFEST 中添加生成事件配置")
		Logging.info("   然后运行 Python 生成脚本: poetry run python tools/generate_orthogonal_events.py")
		return
	
	Logging.info("✅ 找到 %d 个生成事件条目，开始全量解析..." % generated_entries.size())
	...
	if not orphan_csvs.is_empty():
		Logging.info("⚠️  发现 %d 个未收录到 DATA_MANIFEST 的 CSV 文件:" % orphan_csvs.size())
		for oc in orphan_csvs:
			Logging.info("   - %s" % oc)
		Logging.info("   建议将其加入 DATA_MANIFEST（is_generated=true）")
	...
	for entry in generated_entries:
		...
		if csv_path.is_empty():
			Logging.info("⚠️  条目 '%s' 的 save_path 为空，跳过" % name)
			continue
		
		Logging.info("\n--- 处理: %s ---" % name)
		Logging.info("   CSV: %s" % csv_path)
		if not tres_output_dir.is_empty():
			Logging.info("   .tres 输出: %s (手动指定)" % tres_output_dir)
		else:
			Logging.info("   .tres 输出: %s (自动推导自 CSV 路径)" % csv_path.get_base_dir() + "/")
		
		# ── 3. 读取 CSV 内容 ──
		if not FileAccess.file_exists(csv_path):
			Logging.info("⚠️  CSV 文件不存在: %s" % csv_path)
			Logging.info("   请先运行 Python 生成脚本生成此文件")
			continue
		
		var file = FileAccess.open(csv_path, FileAccess.READ)
		if file == null:
			Logging.err("无法打开文件: %s 💀" % csv_path)
			continue
		...
		if csv_content.is_empty():
			Logging.err("CSV 文件为空，跳过: %s 💀" % csv_path)
			continue
		
		Logging.info("✅ 成功读取 CSV (%d 字节)" % csv_content.length())
	...
	Logging.info("\n✅ 生成事件导入完成！")
	Logging.info("   成功处理: %d/%d 个条目" % [success_count, generated_entries.size()])
	if not orphan_csvs.is_empty():
		Logging.info("   ⚠️  目录中还有 %d 个未收录的 CSV（见上方列表）" % orphan_csvs.size())
	Logging.info("   .tres 输出目录: 由各条目的 tres_output_dir 字段指定（详见上方处理日志）")
	Logging.info("===== 📥 导入完成 =====\n")
...
func _clear_godot_import_cache() -> void:
	...
	Logging.info("目标路径:   %s" % godot_dir_path)
	
	# 🛡️ 保险 1：校验路径是否以 .godot 结尾
	if not godot_dir_path.ends_with(".godot"):
		Logging.err("❌ 安全校验失败：目标路径不以 .godot 结尾！路径=%s" % godot_dir_path)
		Logging.err("   拒绝执行，这可能是个错误的目标路径 💀")
		return
	
	# 🛡️ 保险 2：校验父目录是否为项目根目录
	var parent_dir = godot_dir_path.get_base_dir()
	if parent_dir != project_root.trim_suffix("/"):
		Logging.err("❌ 安全校验失败：父目录不是项目根目录！")
		Logging.err("   期望父目录: %s" % project_root)
		Logging.err("   实际父目录: %s" % parent_dir)
		Logging.err("   拒绝执行，路径异常 💀")
		return
	
	# 🛡️ 保险 3：校验目标是否存在
	if not DirAccess.dir_exists_absolute(godot_dir_path):
		Logging.info("⚠️ .godot 文件夹不存在，无需清理")
		return
	
	# 🔥 三重保险全部通过，用 OS.move_to_trash() 安全删除
	Logging.info("🛡️ 安全校验全部通过，开始移动至废纸篓...")
	Logging.info("目标: %s" % godot_dir_path)
	Logging.info("♻️ 使用 OS.move_to_trash() — 文件将被移到系统废纸篓，可随时恢复")
	
	var success = OS.move_to_trash(godot_dir_path)
	
	if success:
		Logging.info("✅ .godot 导入缓存已成功移入废纸篓！")
		Logging.info("⚠️ 重要：请重启 Godot 编辑器让导入机制重新生效")
		Logging.info("   Godot 会在启动时重新生成 .godot/ 目录并全量导入所有资源")
		Logging.info("💡 如需恢复，请前往系统废纸篓找回 .godot 文件夹")
	else:
		Logging.err("❌ 移动至废纸篓失败！可能原因：")
		Logging.err("   - 权限不足（尝试用 rm -rf 手动删除）")
		Logging.err("   - 文件系统错误")
